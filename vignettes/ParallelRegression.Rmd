...
RunExperiment <- function(n, p, s, k){
  ...
  for(i in 1:3){
    ...
    time_vec <- c(time_vec, glm_time[1])
  ...
  for(comm in comm_opts){
    for(i in 1:3){
      new_alg_time <- system.time(new_alg_coeff <- ParLR(X, y, k, comm)$beta)
      diff_vec <- c(diff_vec, norm(beta - new_alg_coeff, "2") / norm(beta, "2"))
      time_vec <- c(time_vec, new_alg_time[1])
    }
  }
...
results <- list()
nSims <- 1
for(n in seq(10000, 20000, by=10000))  {
  for(cores in 1:2){
...
plot_data <- final_results[, .(error = mean(diff), time = median(time)), by=.(n, p, s, k, comm)]
ggplot(plot_data, aes(x = n,
                      y = time,
                      group = interaction(k, comm),
                      color = interaction(k, comm))) +
  geom_line() +
  theme_clean() +
  scale_color_colorblind()
